...
export class Environment {
    ...
    private createClouds(): THREE.Group {
        ...
        for (let i = 0; i < 25; i++) {  // Reduced from 40 to 25
    ...
    private createRuins(): THREE.Group {
        ...
        const createColumn = (x: number, z: number, height: number, fallen: boolean, rotation: number = 0) => {
            const columnRadius = 0.4;
            const columnGeometry = new THREE.CylinderGeometry(columnRadius, columnRadius, height, 16);
        ...
        const createFallenArchitecturalElement = (x: number, z: number, width: number, height: number, depth: number, rotation: number) => {
        ...
        const createFallenColumnSection = (x: number, z: number, length: number, rotation: number) => {
            const columnRadius = 0.4;
            const columnGeometry = new THREE.CylinderGeometry(columnRadius, columnRadius, length, 16);
            const column = new THREE.Mesh(columnGeometry, stoneMaterial);
            column.position.set(x, columnRadius, z);
        ...
        const createRoofPiece = (x: number, z: number, width: number, depth: number, rotation: number) => {
        ...
        const createFallenWallSection = (x: number, z: number, width: number, height: number, rotation: number) => {
    ...
    private createCampfire(): THREE.Group {
        ...
        const createFlame = (height: number, width: number, color: number, x: number, z: number) => {
    ...
    private createTent(): THREE.Group {
        ...
        const indices = new Uint16Array([
            0, 1, 2,          // front
            3, 4, 5,          // back
            6, 7, 8, 6, 8, 9, // left side
            10, 11, 12, 10, 12, 13 // right side
        ]);
    ...
    public updateCampfire(): void {
        ...
        flames.forEach((flame) => {
            ...
            const scaleFluctuation = 0.05 * Math.sin(Date.now() * 0.01 * Math.random());
            flameMesh.scale.x = 1 + scaleFluctuation;
            flameMesh.scale.z = 1 + scaleFluctuation;
            
            // Random height fluctuation
            const heightFluctuation = 0.1 * Math.sin(Date.now() * 0.005 * Math.random());
        ...
        // Animate the lights
        const lights = campfire.children.filter(child => child.type === 'PointLight');
